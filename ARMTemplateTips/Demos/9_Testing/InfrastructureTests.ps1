...
Describe "Networking Tests" -tag "AzureInfrastructure" {
    Context "Networking" {
        $vNet=Get-AzureRmVirtualNetwork -Name "$prefix-vNet" -ResourceGroupName $resourceGroup -ErrorAction SilentlyContinue
        ...
        it "Subnet public-subnet Should Exist" {
            $subnet = Get-AzureRmVirtualNetworkSubnetConfig -Name "public-subnet" -VirtualNetwork $vNet -ErrorAction SilentlyContinue
